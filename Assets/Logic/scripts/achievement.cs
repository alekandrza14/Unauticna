...
public class achievement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //¬зрыв
        if (VarSave.GetBool("¬зрыв"))
        {
            hello.windowmesenge.Dialog_die();
            GetComponent<Text>().text = "Ќравикс(вы) подорвалс€";
            VarSave.SetBool("¬зрыв", false);
        }
        if (VarSave.GetBool("ѕират победил"))
        {
            hello.windowmesenge.Dialog_die();
            GetComponent<Text>().text = "Ѕыл обокраден пиратом";
            VarSave.SetBool("ѕират победил", false);
        }
