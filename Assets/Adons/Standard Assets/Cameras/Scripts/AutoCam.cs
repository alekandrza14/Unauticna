...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] private float m_SpinTurnLimit = 90;// The threshold beyond which the HB_Camera stops following the target's rotation. (used in situations where a car spins out, for example)
        [SerializeField] private float m_TargetVelocityLowerLimit = 4f;// the minimum velocity above which the HB_Camera turns towards the object's velocity. Below this we use the object's forward direction.
        [SerializeField] private float m_SmoothTurnTime = 0.2f; // the smoothing for the HB_Camera's rotation

        private float m_LastFlatAngle; // The relative angle of the target and the rig from the previous frame.
        private float m_CurrentTurnAmount; // How much to turn the HB_Camera
        private float m_TurnSpeedVelocityChange; // The change in the turn speed velocity
        private Vector3 m_RollUp = Vector3.up;// The roll of the HB_Camera around the z axis ( generally this will always just be up )
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            {
                // in follow velocity mode, the HB_Camera's rotation is aligned towards the object's velocity direction
            ...
            {
                // we're in 'follow rotation' mode, where the HB_Camera rig's rotation follows the object's rotation.

                // This section allows the HB_Camera to stop following the target's rotation when the target is spinning too fast.
                // eg when a car has been knocked into a spin. The HB_Camera will resume following the rotation
            ...
            // HB_Camera position moves towards target position:
            transform.position = Vector3.Lerp(transform.position, m_Target.position, deltaTime*m_MoveSpeed);

            // HB_Camera's rotation is split into two parts, which can have independend speed settings:
