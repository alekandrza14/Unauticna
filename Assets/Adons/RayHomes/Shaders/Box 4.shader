Shader "Ray Marching/Box 4"
{
    ...
    CGINCLUDE
        ...
        {
            float3 q = abs(pos) - 0.25;
          float d1 = length(pos-float3(0,0.22f,0))-0.15f;
          float d2 = length(max(q, 0.0f)) + min(max(q.x, max(q.y, q.z)),0.);
            return  SmoothUnion(d1,d2, 4.5f);
        }
    ENDCG
