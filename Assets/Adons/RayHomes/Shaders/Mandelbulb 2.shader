Shader "Ray Marching/Mandelbulb"
{
    ...
    CGINCLUDE
        ...
        {   float c = 0.5;
            float3 pos2 = (pos - float3(0,_Time.y,0));
            
             pos2.y = fmod2(pos2.y + 0.1f * c, c) - 0.1f * c;
            float3 q = abs(pos2/_Scale);
            float3 q2 = (pos/_Scale);
            q -= float3(1.5,0.001,1.5);
            
           float distcube = length(max(q, 0)) + min(max(q.x, max(q.y, q.z)), 0);
           
          float d1 = length(q2)-1.3;
          float d2 = max(d1,distcube);
             if (length(pos) > 4) {
                return Sphere(pos, 0, 2);
            }
            pos = pos/_Scale;
            float3 z = pos;
            float dr = 1.0;
            ...
            float df = _Scale * 0.5 * log(r) * r / dr;
            return SmoothUnion(df,d2*_Scale,10.5);
        }
    ENDCG
