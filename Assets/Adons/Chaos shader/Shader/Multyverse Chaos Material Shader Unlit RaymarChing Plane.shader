Shader "Unauticna Multiverse Customs/Low UnLit Shaders/Multyverse Chaos Raymarching-Material Shader/Plane"
{
    Properties
    {
...
ZWrite off
        ...
        Pass
        {
            ...
            return  min(length(p )- 0.125,p.y+0.125);
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                if (tex.a != 0) {
                   ...
                   if(p.y < -.6) col = lerp(col, tex, 0.6);
                }
                else discard;
                if(p.y > -.6)
                {
                    col*=fixed4(0.5,0,0,0);
                    }else{
                        col*=_MainColor;
                        }
                return col;
