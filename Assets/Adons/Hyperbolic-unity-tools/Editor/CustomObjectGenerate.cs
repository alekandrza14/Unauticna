...
[EditorTool("GenerateCO", typeof(CustomObject))]
public class CustomObjectGenerate : EditorTool
{
    ...
    public override void OnToolGUI(EditorWindow window)
    {
       // HyperbolicPoint trgetpolartransform = ((HyperbolicPoint)target);
        ...
        // Handles.RotationHandle(set, ((GameObject)target).transform.position);
        ((CustomObject)target).datagen();
        if (EditorGUI.EndChangeCheck())
        {

            set = Handles.RotationHandle(set,((GameObject)target).transform.position);
            Undo.RecordObject(target, "GenerateCO");
